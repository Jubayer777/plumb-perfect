...
var serviceAccount = require("./configs/plumb-perfect-firebase-adminsdk-9je2z-ef44983727.json");
admin.initializeApp({
...
client.connect(err => {
  const serviceCollection = client.db("plumbPerfect").collection("services");
  const bookingsCollection = client.db("plumbPerfect").collection("bookings");
  const adminsCollection = client.db("plumbPerfect").collection("admins");
  const reviewsCollection = client.db("plumbPerfect").collection("reviews");
  //add service
